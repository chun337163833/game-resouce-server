...
public class FormModel {
	...
	public String readFormatedValue(String path) {
		TypeDescriptor d = null;
		if(formObject!=null && !(formObject instanceof FilteredList<?>))
			d = ReflectionUtils.getNestedFieldTypeDescriptor(path, formObject);
		if(d==null) {
			return templateHelper.convertToString(readValue(path));
		} else {
			Object v = readValue(path);
			if(v instanceof String)
				return v.toString();
				return templateHelper.convertToString(readValue(path), d);
		}
	}
