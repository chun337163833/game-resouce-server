...
<#macro form commandName baseCaption="_NULL_" action="" isFilter=false method="_NULL_" renderTitle=false renderSaveButton=true class="" customForm=false id="_NULL_" additionalErrors=[] showRequired=true>
...
<#macro input type path renderLabel=true value="_NULL_" defaultCaption="_NULL_" id="_NULL_" class="" spanClass="" labelArguments=[] displayFieldError=true>
	...
	<p>
		...
		<span class="inp-fix <#if _fieldError>inp-error</#if> ${spanClass}">
			<input type="${type}" id="${_id}" name="${path}" class="inp-text class org.apache.maven.project.MavenProject" value="${_value}"/>
		</span>
