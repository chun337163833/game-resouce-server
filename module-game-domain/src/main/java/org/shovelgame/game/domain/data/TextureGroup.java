...
@NamedQueries({ @javax.persistence.NamedQuery(name = "findTexturGroupsGreaterThenVersionGroupByTextureGroup", query = "select e from TextureGroup e left join e.textures as texture where texture.version.value > :version group by e.id") })
public class TextureGroup {
    ...
    public static List<TextureGroup> findTexturGroupsGreaterThenVersionGroupByTextureGroup(BigDecimal version) {
        TypedQuery<TextureGroup> query = entityManager().createNamedQuery("findTexturGroupsGreaterThenVersionGroupByTextureGroup", TextureGroup.class);
