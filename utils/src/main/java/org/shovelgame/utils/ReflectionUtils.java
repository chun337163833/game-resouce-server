...
public class ReflectionUtils extends org.springframework.util.ReflectionUtils {
	...
	public static Object reflectValueSimple(Object data, String key,
		...
		Method method = org.springframework.util.ReflectionUtils.findMethod(
				data.getClass(), getMethodName(amtd.fieldName));
		if (method == null) {
			log.warn("getter not found for " + key + " in class "
					+ data.getClass());
			return null;
		}
		...
		try {
			Object _result = org.springframework.util.ReflectionUtils
	...
	public static void setNestedValue(String path, Object instance, Object value)
			throws Exception {
	...
	public static Field getDeclaredFieldFromAll(String name, Class<?> clz)
	...
	public static TypeDescriptor getNestedFieldTypeDescriptor(String path,
			Object instance) {
		String[] keys = path.split("\\.");
	...
	public static void setValue(String path, Object instance, Object value)
			throws Exception {
		Field f = findDeclaredField(instance.getClass(), path);
		if (f == null) {
			log.warn(String.format("No field $1%s found on class $2%s", path,
	...
	public static Field findDeclaredField(Class<?> instance, String fieldName)
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				classes.addAll(findClasses(file,
					...
					&& !ignoreClass(file.getName(), ignore)) {
	...
	/**
	 * key can be defined by dot notation. such as
	 * entity.employee.department.longName
	 * 
	 * @param data
	 * @param key
	 * @return
	...
	public static <T> boolean hasInterface(Class<?> dataClass,
	...
	public static <T> boolean isChildOf(Class<?> dataClass,
			Class<T> requiredClass) {
		return isChildOf(dataClass, requiredClass, true);
	}

	public static <T> boolean isChildOf(Class<?> dataClass,
			Class<T> requiredClass, boolean isInterface) {
		boolean result = dataClass.equals(requiredClass);
		/*
		 * Object.class je posledni mozna dedicnost a tu uz kontrolovat
		 * nepotrebujeme. Nevim jestli se to takhle dela spravne, ale nic jineho
		 * me nenapadlo.
		 */
	...
	public static List<Class<?>> findClasses(Class<? extends Annotation> annotation, String... packageNames) {
		ConfigurationBuilder config = ConfigurationBuilder.build(new Object[]{packageNames});
	...
	public static List<Class<?>> findClasses(String... packageNames) {
		return findClasses(null, packageNames);
	}
